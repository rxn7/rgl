...
void rglStart(rglAppDesc *desc) {
  ...
  if (!desc->title)
    desc->title = "RGL";
  if (!desc->update_f)
    desc->update_f = _rglDefaultUpdateFunc;
  if (!desc->draw_f)
    desc->draw_f = _rglDefaultDrawFunc;
...
void rglSetVsync(b8 value) { RGL_PLATFORM_FUN(SetVsync, value); }
...
f32 rglGetTime(void) { return RGL_PLATFORM_FUN(GetTime); }
...
void _rglMainLoop(void) {
  ...
  while (_rgl_running) {
    ...
    glClearColor((f32)_rgl_app_desc->background_color.r / 255.f,
                 (f32)_rgl_app_desc->background_color.g / 255.f,
                 (f32)_rgl_app_desc->background_color.b / 255.f, 1.f);
...
void _rglDefaultUpdateFunc(f32 dt) { return; }

void _rglDefaultDrawFunc(void) { return; }
