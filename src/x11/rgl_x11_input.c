...
static u32 _rglBtnToMask(rglBtn btn) {
  switch (btn) {
  case RGL_MOUSE_LEFT:
    return Button1Mask;
  case RGL_MOUSE_MIDDLE:
    return Button2Mask;
  case RGL_MOUSE_RIGHT:
    return Button3Mask;
  default:
    break;
  }
...
void rglX11InputUpdate(void) {
  if (_rgl_plat_ctx->focus) {
    ...
    _cursor_pos.x = mx; _cursor_pos.y = my;
...
void rglX11GetCursorPos(rglV2 *vec) { rglV2Copy(&_cursor_pos, vec); }
...
b8 rglX11IsKeyJustPressed(rglKey key) {
  KeyCode keycode = XKeysymToKeycode(_rgl_plat_ctx->dpy, _rglKeyToKeysym(key));
  return (_keys[keycode / 8] & (1 << (keycode % 8))) != 0 &&
         (_prev_keys[keycode / 8] & (1 << (keycode % 8))) == 0;
}
...
b8 rglX11IsBtnPressed(rglBtn btn) { return _buttons & _rglBtnToMask(btn); }
